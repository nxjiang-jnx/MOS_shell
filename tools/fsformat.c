...
void init_disk() {
	...
	nbitblock = (NBLOCK + BLOCK_SIZE_BIT - 1) / BLOCK_SIZE_BIT;		// 用1个存储磁盘块就够了
	nextbno = 2 + nbitblock;		// 第一个可用块编号，即3
...
struct File *create_file(struct File *dirf) {
	...
	// Step 1: Iterate through all existing blocks in the directory. 找有没有空闲block
