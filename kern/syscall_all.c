...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if (is_illegal_va_range(va, len) || is_illegal_dev_range(pa, len) || va % len != 0) {	// 模板
		return -E_INVAL;
	}
	if (len == 4) {
...
int sys_get_set_globalVar(char* name, char* val, int mode, int id, int rwMode) {
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_set_env_curpath] = sys_set_env_curpath,
	[SYS_get_set_globalVar] = sys_get_set_globalVar,
	[SYS_get_parentid] = sys_get_parentid,
};
