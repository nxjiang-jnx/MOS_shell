...
int page_alloc(struct Page **new) {
	// 该函数只要调用都会分配。成功返回0，失败返回 -E_NO_MEM, 通常使用方法：if (page_alloc(&pp) == -E_NO_MEM)
	...
	// 注意这里 page2kva 返回的是 u_long 类型，把这个返回值看成地址。因此是类型转换而不是取该返回值的地址。
	memset((void*)page2kva(pp), 0, PAGE_SIZE);		// 通过内核虚拟地址（高地址）访问或清空物理内存
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	Pde *pgdir_entryp;
	struct Page *pp;	// page pointer，作为新建的二级页表项
