...
int syscall_get_set_globalVar(char* name, char* val,  int mode, int id, int rwMode) {
	return msyscall(SYS_get_set_globalVar, name, val, mode, id, rwMode);
}

u_int syscall_get_parentid(void) {
	return msyscall(SYS_get_parentid);
}