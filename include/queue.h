 ...
 * head是指针！
 * 返回的是指向第一个元素的指针。通常用于分配 page_free_list 头部页。
 * 即：pp = LIST_FIRST(&page_free_list);
 */
 ...
 * 注意传入的 head 是指针类型！一般调用时都是 LIST_INIT(&...)
 */
 ...
 * 注意：
 * head 是首指针，通常为 &...
 * elm 是指向要插入元素的指针，这个元素是完备的，既包含链表项又包含内容。如 &pages[i]
 * field 是链表项的字段。通常是 pp_link
 */
